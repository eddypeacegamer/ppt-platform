...
public interface PagoRepository extends JpaRepository<Pago, Integer> {
	...
	@Query("select p from Pago p where p.tipoPago='INGRESO' and p.statusPago like upper(:status) and p.formaPago like upper(:formaPago) and p.banco like upper(:banco) and  p.cuenta like upper(:cuenta) and p.fechaPago between :since and :to")
	...
	@Query("select p from Pago p where p.tipoPago='EGRESO' and p.statusPago like upper(:status) and p.formaPago like upper(:formaPago) and p.banco like upper(:banco) and  p.cuenta like upper(:cuenta) and p.fechaCreacion between :since and :to")
	...
	@Query("select sum(p.monto) from Pago p where p.tipoPago='INGRESO' and p.statusPago='ACEPTADO' and p.formaPago like upper(:formaPago) and p.banco like upper(:banco) and  p.cuenta like upper(:cuenta) and p.fechaPago between :since and :to")
	...
	@Query("select sum(p.monto) from Pago p where p.tipoPago='EGRESO' and p.statusPago='PAGADO' and p.formaPago like upper(:formaPago) and p.banco like upper(:banco) and  p.cuenta like upper(:cuenta) and p.fechaCreacion between :since and :to")
