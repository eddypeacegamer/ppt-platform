...
public class FacturaDao {
	...
	private static final String  INVOICE_BY_FOLIO = "SELECT f.FOLIO,f.UUID,cfdi.FECHA,f.RAZON_SOCIAL_EMISOR,f.RFC_EMISOR,f.RAZON_SOCIAL_REMITENTE,f.RFC_REMITENTE,f.TIPO_DOCUMENTO,f.PACK_FACTURACION,cfdi.TIPO_COMPROBANTE,"
					...
					+"ctos.CLAVE_PROD_SERV,ctos.DESCRIPCION,ctos.VALOR_UNITARIO,ctos.IMPORTE FROM FACTURAS f INNER JOIN CFDI cfdi ON f.ID_CFDI = cfdi.ID_CFDI INNER JOIN CFDI_CONCEPTOS ctos ON cfdi.ID_CFDI = ctos.ID_CFDI "
					+"WHERE cfdi.FOLIO= ?";
	
	private static final String  INVOICES_BY_FOLIOS = "SELECT f.FOLIO,f.UUID,cfdi.FECHA,f.RAZON_SOCIAL_EMISOR,f.RFC_EMISOR,f.RAZON_SOCIAL_REMITENTE,f.RFC_REMITENTE,f.TIPO_DOCUMENTO,f.PACK_FACTURACION,cfdi.TIPO_COMPROBANTE,"
			+"cfdi.IMP_TRASLADADOS,cfdi.IMP_RETENIDOS,cfdi.SUB_TOTAL,cfdi.TOTAL,cfdi.METODO_PAGO,cfdi.FORMA_PAGO,cfdi.MONEDA,f.STATUS_PAGO,f.STATUS_FACTURA,f.FECHA_CANCELADO,ctos.CANTIDAD,ctos.CLAVE_UNIDAD,ctos.UNIDAD,"
			+"ctos.CLAVE_PROD_SERV,ctos.DESCRIPCION,ctos.VALOR_UNITARIO,ctos.IMPORTE FROM FACTURAS f INNER JOIN CFDI cfdi ON f.ID_CFDI = cfdi.ID_CFDI INNER JOIN CFDI_CONCEPTOS ctos ON cfdi.ID_CFDI = ctos.ID_CFDI "
	...
	private static final String COMPLEMENT_BY_FOLIO = "SELECT f.FOLIO_PADRE AS FOLIO,p.ID_DOCUMENTO AS UUID,cfdi.FECHA,f.RAZON_SOCIAL_EMISOR,f.RFC_EMISOR,f.RAZON_SOCIAL_REMITENTE,f.RFC_REMITENTE,f.TIPO_DOCUMENTO,f.PACK_FACTURACION,"
			...
			+ "p.IMPORTE_PAGADO,p.IMPORTE_SALDO_ANTERIOR,p.IMPORTE_SALDO_INSOLUTO,p.NUM_PARCIALIDAD FROM FACTURAS f INNER JOIN CFDI cfdi ON f.ID_CFDI = cfdi.ID_CFDI INNER JOIN CFDI_PAGOS p ON cfdi.ID_CFDI = p.ID_CFDI "
			+"WHERE cfdi.FOLIO= ?";
